...
def get_predictions(model, tokenizer, collate_fn, dataset):
    ...
    for instance in tqdm(dataset):
        ...
        for i, sent in enumerate(instance['text']):
            perturbs = []
            if str(i) not in instance['orig_sent_perturbs']:
                # the parse has failed during constituent occlusion
                continue
            filtered_perturbs = []
            for perturb in instance['orig_sent_perturbs'][str(i)]:
                if not perturb['new_sent'] or not perturb['new_sent'] or len(perturb['new_sent'])<=1 or not perturb['new_sent'][1]:
                    continue
                else:
                    filtered_perturbs.append(perturb)
            instance['orig_sent_perturbs'][str(i)] = filtered_perturbs

            for perturb in instance['orig_sent_perturbs'][str(i)]:
                new_instance = deepcopy(instance)
                new_instance['text'][i][1] = perturb['new_sent']
                perturbs.append(new_instance)

            # remove the whole sentence as well
            ...
            for j in range(len(instance['orig_sent_perturbs'][str(i)])):
                instance['orig_sent_perturbs'][str(i)][j][f'pred_{args.model_name}'] = preds[j]
                instance['orig_sent_perturbs'][str(i)][j][f'logits_{args.model_name}'] = logits[j]
                instance['orig_sent_perturbs'][str(i)][j][f'logit_pred_{args.model_name}'] = logit_pred[j]
...
if __name__ == "__main__":
    ...
    train = get_predictions(model, tokenizer, collate_fn, train)
    with open(f'data/{args.dataset}_{args.model_name}_train_precomputed_predictions.json', 'w') as out:
        json.dump(train.dataset, out)



"""
python3.8 modeling/precompute_predictions.py --gpu --dataset fever --dataset_dir /image/image-copenlu/generating-adversarial-claims/data/ --model_path models/fever_oracle_bert_1e5 --labels 3 --batch_size 8 --model_name bert_1
